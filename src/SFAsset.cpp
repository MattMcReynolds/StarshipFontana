...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
  ...
  case SFASSET_COIN:
    ...
    break;
  case SFASSET_BAR:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/bar.png");
    break;
  case SFASSET_BASE:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/base.png");
    break;
  }
...
Point2 SFAsset::GetPosition() {
  return Point2(bbox->centre->getX(), bbox->centre->getY()+30);
}
...
void SFAsset::WiggleTime() {
  int move = rand() % 10; //randomises a one in ten chance of moving
  int w, h;
  SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
  int decision = rand() % 2;  //randomises left or right (0 or 1)
  Vector2 c = *(bbox->centre) + Vector2(40.0f, 0.0f);
  if (decision == 0)
  {
    c = *(bbox->centre) + Vector2(-40.0f, 0.0f);
  }
  if(!(c.getX()-30.0f < 0)&&!(c.getX()+30.0f > w)&&move==0) {
    bbox->centre.reset();
...
void SFAsset::BulletNorth() { //Separate speed function for bullets
  int w, h;
  SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
...
void SFAsset::GoSouthSlow() {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, -1.3f);
  if(!(c.getY() < 0)) {
    bbox->centre.reset();
    bbox->centre = make_shared<Vector2>(c);
  }
